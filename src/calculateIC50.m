...
function [mratios,errs,raw_ratios,IC50,ci,rsq2] = calculateIC50(dirname,empty_well,var_well,timepoint,egf)
    ...
    well1_ratio = intensities(fret_im_files(1,:),cfp_im_files(1,:),empty_fret_files{1});
    well2_ratio = intensities(fret_im_files(2,:),cfp_im_files(2,:),empty_fret_files{2});
    well3_ratio = intensities(fret_im_files(3,:),cfp_im_files(3,:),empty_fret_files{3});
    well4_ratio = intensities(fret_im_files(4,:),cfp_im_files(4,:),empty_fret_files{4});
